...
const blockType = extendContent(Block, "KongJianQiao2", {
   // isPlaceable() { return dsGlobal.techDsAvailable() && this.super$isPlaceable(); },
    load() {
        this.super$load();
        topRegion = lib.loadRegion("KongJianQiao2-top");
        bottomRegion = lib.loadRegion("KongJianQiao2-bottom");
        rotatorRegion = lib.loadRegion("KongJianQiao2-rotator");
    },
    setBars() {
        this.super$setBars();
...
blockType.buildType = prov(() => {
    ...
    return extend(Building, {
        ...
        buildConfiguration(table) {
        	...
        	var items = table.table().update(t => t.visible = shownConfig).get();
            ItemSelection.buildTable(items, Vars.content.items(), prov(() => itemType), cons(v => {
                this.configure(v);
            }));
        },
        config() {
